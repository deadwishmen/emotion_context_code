...
def get_args():
    ...
    parser.add_argument('--combine', default='default', type=str, choices=['default', 'q_former'], help="Combination method")
...
def load_models(args):
    ...
    # Context model
    if args.combine == "q_former" or args.model_context == "vit":
    ...
    num_context_features = 768 if args.combine == "q_former" or args.model_context == "vit" else list(model_context.children())[-1].in_features
    ...
    fusion_class = fusion_model_map.get(args.combine if args.combine != "default" else "concat")
    if args.combine in ["sum", "avg"]:
        fusion_model = fusion_class(num_context_features, num_body_features, num_face_features, num_text_features, args.combine)
    else:
...
def main():
    ...
    # Run test
    test_map = test_disc(models, device, test_loader, test_length, args, combine=args.combine, xai=args.xai)
